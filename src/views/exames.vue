      ...
      <v-row>
        <v-col cols="12" sm="6" md="4" v-for="i in exameList" :key="i.id">
          <v-card :color="i.available == 1 ? 'green lighten-4' : 'red lighten-4'" class="ma-2">
            ...
            <v-card-actions>
              <v-btn color="primary" :to="{name:'QuestionsView', params:{uuid:i.uuid}}">
                Detalhes
              </v-btn>
              <v-spacer></v-spacer>
...
export default defineComponent({
  ...
  methods: {
    ...
    async insertExam() {
      try {
        await apiService.insert(`${baseurl}/insert`, { uuid: uuidv4() });
    ...
    async toggleExameStatus(exame: { id: number; available: number }) {
      try {
        ...
        await apiService.update(`${baseurl}/update/${exame.id}`, { available: newStatus });
